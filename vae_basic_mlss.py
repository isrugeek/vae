...
def vae(observed, n, n_x, n_z, n_samples, is_training):
    with zs.BayesianNet(observed=observed) as model:
        ...
        # OPTION 2  for applying Bernoulli method I can use either modes

        #z = zs.Bernoulli('x',z_mean, z_std, n_samples=n_samples, group_event_ndims =1)



        lx_z_1 = layers.fully_connected(z, 500, normalizer_fn=layers.batch_norm,normalizer_params=normalizer_params) # a mlp layer of 500 hidden units with z as the input
        ...
        lx_z_2 = layers.fully_connected(lx_z_1 , 500,normalizer_fn=layers.batch_norm,normalizer_params=normalizer_params)
...
def q_net(observed, x, n_z, n_samples, is_training):
    with zs.BayesianNet(observed=observed) as variational:
        ...
        lz_x_1 = layers.fully_connected(x, 500,normalizer_fn=layers.batch_norm,normalizer_params=normalizer_params)
        lz_x_2 = layers.fully_connected(lz_x_1, 500, normalizer_fn=layers.batch_norm,normalizer_params=normalizer_params)
        ...
        # Anoyther method for applying method
...
if __name__ == "__main__":
    ...
    lower_bound = zs.variational.elbo(log_joint, {'x': x_obs}, {'z': [qz_samples, log_qz]},axis =0)
    ...
    # Run the inference
    gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.9) # use only part of the gpu
